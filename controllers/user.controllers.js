  ...
  try {
    ...
    if (foundUser) {
      return res.status(400).send({
        error: [{ msg: 'user already exist email should be unique' }],
      })
    }
...
const SignIn = async (req, res) => {
  try {
    ...
    console.log(error)
    res.status(400).send({ error: [{ msg: 'can not get the currentUser' }] })
  }
}
...
const adduser = async (req, res) => {
  try {
    ...
    if (!newUser.name || !newUser.email) {
      res.status(400).send({ error: [{ msg: 'name and email are required' }] })
      return
    }
    ...
    res.status(500).send({ error: [{ msg: 'can not add this user' }] })
  }
}
...
const getusers = async (req, res) => {
  try {
    ...
    res.status(404).send({ error: [{ msg: 'can not get users' }] })
  }
}
...
const getuser = async (req, res) => {
  try {
    ...
    res.status(404).send({ error: [{ msg: 'can not get user with this id' }] })
  }
}
...
const deleteuser = async (req, res) => {
  ...
  try {
    ...
    res.status(404).send({ error: [{ msg: 'can not delete this user' }] })
  }
}
...
const updateuser = async (req, res) => {
  try {
    ...
    res.status(404).send({ error: [{ msg: 'can not update this user' }] })
  }
}
